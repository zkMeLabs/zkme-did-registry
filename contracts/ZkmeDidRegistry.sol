...
/**
 *@title ZkmeDidRegistry
 *@dev Smart Contract for Zkme DID Method
 */
contract ZkmeDidRegistry {
    uint256 totalDIDs;
    address owner;
    uint256 deletedDID;
    struct ZkmeDID {
    ...
    modifier onlyController(address _id) {
        require(
            zkmeDIDs[_id].controller == msg.sender,
            "message sender is not the controller of the DID Doc"
        );
        _;
    }
    mapping(address => ZkmeDID) zkmeDIDs;
    ...
    {
        zkmeDIDs[_id].controller = msg.sender;
        zkmeDIDs[_id].created = block.timestamp;
        zkmeDIDs[_id].updated = block.timestamp;
        zkmeDIDs[_id].didDoc = _doc;
        activeDIDs[totalDIDs] = msg.sender;
        activeAddress[_id] = totalDIDs;
        ++totalDIDs;
        emit DIDCreated(_id, _doc);
        return (
            zkmeDIDs[_id].controller,
            zkmeDIDs[_id].created,
            zkmeDIDs[_id].updated,
            zkmeDIDs[_id].didDoc
        );
    }
    ...
    function getDIDDOcByIndex(
        ...
        return zkmeDIDs[activeDIDs[_index]].didDoc;
    }
